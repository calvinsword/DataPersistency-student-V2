...
SELECT mnr, functie, gbdatum FROM medewerkers WHERE gbdatum < '1980-01-01' AND functie IN ('TRAINER','VERKOPER');
...
WHERE EXTRACT(YEAR FROM uitvoeringen.begindatum) = 2019
GROUP BY uitvoeringen.cursus, uitvoeringen.begindatum
HAVING COUNT(inschrijvingen.cursus) >= 3;
...
SELECT
    EXTRACT(YEAR FROM AGE(MAX(gbdatum), MIN(gbdatum))) AS verschil,
    AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, gbdatum))) AS gemiddeld
FROM medewerkers;
